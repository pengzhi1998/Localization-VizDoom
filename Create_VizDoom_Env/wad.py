...
def build_wall(maze):
    ...
    def __add_start(w, h):
        ...
        # things.append(ZThing(*[len(things) + 1000, x, y, 0, 0, 9001, 0]))
    ...
    for h, row in enumerate(maze):
        # print(row.strip())
        for w, block in enumerate(row.strip()): # see what row.strip, h and w is
            if block == '1':
                __add_vertex(w, h)
            else:
                pass
...
def main(flags):
    ...
    for map_index, file_name in enumerate(
            glob.glob('{}_*.txt'.format(flags.prefix))): # 只遍历命名提到的txt file，
            # 可以将*改为特定的map，比如"7_TEST_MAP52.txt"这样的形式
        # print(map_index, file_name)
        ...
        new_wad.maps['MAP{}'.format(map_index)] = new_map.to_lumps()
