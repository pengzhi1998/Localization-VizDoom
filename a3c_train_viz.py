...
def train(rank, args, shared_model): # rank is referring to the serial number of the num_processes
    ...
    while num_iters < args.num_iters/20000:
        ...
        if done:
            # filling action history with action 3 at the start of the episode, that the
            # action_hist will be [3, 3, 3, 3, 3] (for the default action_hist_size), and the length will be 5.
            # from action_hist[0] to action_hist[-1] are the actions from the oldest to the newest.
            action_hist = deque(
                [3] * action_hist_size,
        ...
        for step in range(args.num_steps):
            ...
            # value for the critic, logit for the actor, and the ax represents
            # for actions it has took, dx represents the states
            value, logit = model(
            ...
            '''
            Need to add the num_samples into the multinomial. And action is a tensor,
            for example, tensor([[1]]), represents for the action it is chosen.
            '''
            action = prob.multinomial(1).data
            log_prob = log_prob.gather(1, Variable(action))

            # print("prob:", prob, "entropy:", entropy, "action:", action, "log_prob:", log_prob)
